...
def filter_gan_masks(img_path, msk_path, out_path, threshold_method=threshold_li, do_watershed_and_four_connectivity=True, gaussian_blur_amount=0.0, dark_background=True):
    for f in tqdm(os.listdir(img_path)):
        img = np.array(Image.open(os.path.join(img_path, f)), dtype='uint8')
        mask = np.array(Image.open(os.path.join(msk_path, f)), dtype='uint8')
        if do_watershed_and_four_connectivity:
            mask = segment(image=mask, threshold=-1, watershed_lines=True, use_four_connectivity=True)
        m = Measure(mask, darkBackground=dark_background, applyWatershed=False, excludeEdges=False, grayscaleImage=img)
        m.calculateMeanIntensities()
        
        if dark_background:
            m.filterResults('meanIntensity', minValue=threshold_method(img))
        else:
            m.filterResults('meanIntensity', maxValue=threshold_method(img))
        ...
        if gaussian_blur_amount > 0:
            ...
            img = img.filter(PIL.ImageFilter.GaussianBlur(gaussian_blur_amount))
            img.save(os.path.join(img_path, f))
...
def prepare_images_cycle_gan(root_dir, input_dir_images, tile_size_w=384, tile_size_h=384, num_simulated_masks=1000, dark_background=True):
    ...
    for i, input_img in enumerate(input_imgs):
        ...
        for j, img_tile in enumerate(img_tiles):
            ...
            if (dark_background and np.mean(img_tile) >= 1.1 * np.mean(input_img)) or (not dark_background and np.mean(img_tile) <= 0.9 * np.mean(input_img)):
...
def load_and_preprocess_images(input_dir_or_filelist, threshold_value=None, normalization_range=(-1, 1), output_channels=1, contrast_optimization_range=None):
    ...
    for file in file_list:
        image = np.array(Image.open(file), dtype='float32')
