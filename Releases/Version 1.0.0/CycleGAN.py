from tensorflow.keras.layers import Dense, Layer, Input, Conv2D, Activation, add, UpSampling2D,Conv2DTranspose, Flatten, AveragePooling2D, LeakyReLU, InputSpec
...
class CycleGAN():
    def loadData(self):
        ...
        if self.use_data_generator:
            self.data_generator = load_data.load_data(
                nr_of_channels=self.channels, batch_size=self.batch_size, generator=True, subfolder=self.image_folder)
    ...
    def runTraining(self):
        self.image_folder = os.path.join(self.ROOT_DIR, 'data')
        self.loadData()

        # ======= Create designated run folder and store meta data ==========
        directory = os.path.join(self.ROOT_DIR, 'images', self.date_time)
    ...
    def runInference(self):
        self.image_folder = os.path.join(self.ROOT_DIR, 'generate_images', 'synthetic_images')
        self.loadData()
    ...
    def __init__(self, lr_D=2e-4, lr_G=2e-4, image_shape=(352, 512, 1),
                 date_time_string_addition='',
                 ROOT_DIR='./'
                 ):

        self.ROOT_DIR = os.path.join(ROOT_DIR, '2_CycleGAN')
        ...
        self.decay_epoch = 201  # The epoch where the linear decay of the learning rates start
        ...
        config = tf.compat.v1.ConfigProto(gpu_options=tf.compat.v1.GPUOptions(per_process_gpu_memory_fraction=0.2))
...
# Training
    def train(self, epochs, batch_size=1, save_interval=1):
        ...
        for epoch in range(1, epochs + 1):
            ...
            training_history = {
                'DA_losses': np.average(np.asarray(DA_losses)),
                'DB_losses': np.average(np.asarray(DB_losses)),
                'gA_d_losses_synthetic': np.average(np.asarray(gA_d_losses_synthetic)),
                'gB_d_losses_synthetic': np.average(np.asarray(gB_d_losses_synthetic)),
                'gA_losses_reconstructed': np.average(np.asarray(gA_losses_reconstructed)),
                'gB_losses_reconstructed': np.average(np.asarray(gB_losses_reconstructed)),
                'D_losses': np.average(np.asarray(D_losses)),
                'G_losses': np.average(np.asarray(G_losses), axis=0),
                'reconstruction_losses': np.average(np.asarray(reconstruction_losses))}
            self.writeLossDataToFile(training_history)

            # Save best model checkpoint for later inference
            if best_D_loss is None:
                best_D_loss = np.average(np.asarray(D_losses))
            if np.average(np.asarray(D_losses)) <= best_D_loss:
                print('Discriminator Loss improved from {} to {}. Saving checkpoint.'.format(best_D_loss, np.average(np.asarray(D_losses))))
                self.saveModel(self.G_A2B, epoch, saving_checkpoint=True)
                self.saveModel(self.G_B2A, epoch, saving_checkpoint=True)
                best_D_loss = np.average(np.asarray(D_losses))
            else:
                print('Discriminator Loss {} did not improve from {}.'.format(np.average(np.asarray(D_losses)), best_D_loss))
...
# Help functions
    ...
    def lse(self, y_true, y_pred):
        loss = tf.math.reduce_mean(tf.math.squared_difference(y_pred, y_true))
        return loss

    def cycle_loss(self, y_true, y_pred):
        loss = tf.math.reduce_mean(tf.math.abs(y_pred - y_true))
        return loss
    ...
    def truncateAndSave(self, real_, real, synthetic, reconstructed, path_name):
        ...
        imageio.imwrite(path_name, np.asarray((image-image.min())/(image.max()-image.min())*255, dtype='uint8'))
    ...
    def saveImages(self, epoch, real_image_A, real_image_B, num_saved_images=1):
        ...
        for i in range(num_saved_images + 1):
            ...
            self.truncateAndSave(real_image_Ab, real_image_A, synthetic_image_B, reconstructed_image_A,
                                 self.ROOT_DIR + '/images/{}/{}/epoch{}_sample{}.png'.format(
    ...
    def save_tmp_images(self, real_image_A, real_image_B, synthetic_image_A, synthetic_image_B):
        try:
            ...
            self.truncateAndSave(None, real_images, synthetic_images, reconstructed_images,
                                 self.ROOT_DIR + '/images/{}/{}.png'.format(
                                     self.date_time, 'tmp'))
...
# Save and load
    ...
    def saveModel(self, model, epoch, saving_checkpoint=False):
        # Create folder to save model architecture and weights
        directory = os.path.join(self.ROOT_DIR, 'saved_models', self.date_time)
        if not os.path.exists(directory):
            os.makedirs(directory)

        if not saving_checkpoint:
            model_path_w = self.ROOT_DIR + '/saved_models/{}/{}_weights_epoch_{}.hdf5'.format(self.date_time, model.name, epoch)
            model.save_weights(model_path_w)
            model_path_m = self.ROOT_DIR + '/saved_models/{}/{}_model_epoch_{}.json'.format(self.date_time, model.name, epoch)
            ...
            model.save_weights(os.path.join(self.ROOT_DIR, 'generate_images', 'models', '{}.hdf5'.format(model.name)))

    def writeLossDataToFile(self, history):
        write_header = not os.path.isfile(self.ROOT_DIR + '/images/{}/loss_output.csv'.format(self.date_time))
        keys = sorted(history.keys())
        with open(self.ROOT_DIR + '/images/{}/loss_output.csv'.format(self.date_time), 'a') as csv_file:
            writer = csv.writer(csv_file, delimiter=',', lineterminator='\n')
            if write_header:
                writer.writerow(keys)
            writer.writerow([history[key] for key in keys])
    ...
    def writeMetaDataToJSON(self):
        ...
        with open(self.ROOT_DIR + '/images/{}/meta_data.json'.format(self.date_time), 'w') as outfile:
            json.dump(data, outfile, sort_keys=True)

    def load_model_and_weights(self, model):
        path_to_model = os.path.join(self.ROOT_DIR, 'generate_images', 'models', '{}.json'.format(model.name))
        path_to_weights = os.path.join(self.ROOT_DIR, 'generate_images', 'models', '{}.hdf5'.format(model.name))
        ...
        if response == 'y':
            ...
            def save_image(image, name, domain):
                if self.channels == 1:
                    ...
                    #toimage(image, cmin=-1, cmax=1).save(os.path.join('generate_images', 'synthetic_images', domain, name))
                    imageio.imwrite(os.path.join(self.ROOT_DIR, 'generate_images', 'synthetic_images', domain, name), np.asarray((image-image.min())/(image.max()-image.min())*255, dtype='uint8'))

            # Test A images
            for i in range(len(synthetic_images_A)):
                # Get the name from the image it was conditioned on
                name = self.testB_image_names[i].strip('.tif') + '_synthetic.tif'
            ...
            for i in range(len(synthetic_images_B)):
                # Get the name from the image it was conditioned on
                name = self.testA_image_names[i].strip('.tif') + '_synthetic.tif'
...
if __name__ == '__main__':
    ...
    cycleGAN = CycleGAN(image_shape=image_shape, ROOT_DIR=ROOT_DIR)
