...
class myDataset():
    ...
    def initialize_images(self, subset, source):
        ...
        source: Are the masks from SEM images, TSEM images, or simulated by a GAN?
        """
        ...
        if (subset == "train"):
            if source == 'SEM' or source == 'TSEM':
                allImages.remove(os.path.join(self.IMAGE_DIR, '1908256.tif'))
                allImages.remove(os.path.join(self.IMAGE_DIR, '1908268.tif'))
                allImages.remove(os.path.join(self.IMAGE_DIR, '1908282.tif'))
                allImages.remove(os.path.join(self.IMAGE_DIR, '1908320.tif'))
                allImages.remove(os.path.join(self.IMAGE_DIR, '1908326.tif'))
                allImages.remove(os.path.join(self.IMAGE_DIR, '1908324.tif'))
            else:
                allImages = allImages[:int(0.8*len(allImages))]

        elif (subset == "val"):
            if source == 'SEM' or source == 'TSEM':
                allImages = [os.path.join(self.IMAGE_DIR, '1908256.tif'),
                             os.path.join(self.IMAGE_DIR, '1908268.tif'),
                             os.path.join(self.IMAGE_DIR, '1908282.tif'),
                             os.path.join(self.IMAGE_DIR, '1908320.tif'),
                             os.path.join(self.IMAGE_DIR, '1908326.tif'),
                             os.path.join(self.IMAGE_DIR, '1908324.tif')
                             ]
            else:
                allImages = allImages[int(0.8 * len(allImages)):]
        ...
        for i in range(0, len(allImages)):
            ...
            if self.source == 'SEM':
                mask_path = image_path.replace(self.IMAGE_DIR, self.MASK_DIR).replace(".tif", "_m.tif")
            elif self.source == 'TSEM':
                mask_path = image_path.replace(self.IMAGE_DIR, self.MASK_DIR).replace(os.path.basename(image_path), str(int(os.path.basename(image_path).replace(".tif", "")) + 1) + ".tif")
            else:
            ...
            offH = 0
            if self.source == 'TSEM':
                transformFile = open(os.path.join(os.path.dirname(mask_path), "Image_Registration", "Transform", os.path.basename(mask_path).replace(".tif", ".xml")))
                for lines in transformFile:
                    if "data=" in lines:
                        offW, offH = lines[lines.find("data=") + len("data="):lines.find("/>")].replace('"', '').strip().split(" ")
                        offW = round(float(offW), 0)
                        offH = round(float(offH), 0)
                        self.maxOffW = max(self.maxOffW, abs(int(offW)))
                        self.maxOffH = max(self.maxOffH, abs(int(offH)))
                        break
                transformFile.close()
    ...
    def load_image(self, image_id):
        ...
        source = info['source']
        offH = float(info['offsetH'])
        offW = float(info['offsetW'])
        OFFSET_H = int(max(0, offH))
        OFFSET_W = int(max(0, offW))
        ...
        if source == 'TSEM':
            # Correct for drift between TSEM and SEM image
            tmp = np.zeros((self.IMAGE_SIZE_H, self.IMAGE_SIZE_W), dtype='float32')
            tmp[0:self.IMAGE_SIZE_H - self.maxOffH, 0:self.IMAGE_SIZE_W - self.maxOffW] = image[OFFSET_H:OFFSET_H + self.IMAGE_SIZE_H - self.maxOffH, OFFSET_W:OFFSET_W + self.IMAGE_SIZE_W - self.maxOffW]
            image = tmp
    ...
    def load_mask(self, image_id):
        ...
        if source == 'TSEM':
            # Correct Offsets of SEM and TSEM
            tmp = np.zeros((self.IMAGE_SIZE_H, self.IMAGE_SIZE_W), dtype='float32')
            tmp[0:self.IMAGE_SIZE_H - self.maxOffH, 0:self.IMAGE_SIZE_W - self.maxOffW] = mask[OFFSET_H:OFFSET_H + self.IMAGE_SIZE_H - self.maxOffH, OFFSET_W:OFFSET_W + self.IMAGE_SIZE_W - self.maxOffW]
            mask = tmp
