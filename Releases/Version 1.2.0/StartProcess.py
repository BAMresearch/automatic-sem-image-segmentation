...
USE_GPUS_NO = 2                                                             # List of GPUs used for training (if there is more than one available)
...
MAX_PARTICLE_OVERLAP = 0.5                                                  # Maximum overlap of particles in simulated masks (fraction of particle area) - Use a medium to high value for SEM images (e.g., 0.5) and a small value for TEM images (e.g., 0.05)
CYCLEGAN_BATCH_SIZE = 5                                                     # Batch size used during CycleGAN training
CYCLEGAN_EPOCHS = 50                                                        # Number of training epochs for CycleGAN
CYCLEGAN_USE_SKIPS = False                                                  # Use skip connections in CycleGAN (conceptually somewhat similar to identity mappings)
CYCLEGAN_FILTERS = 64                                                       # Number of filters in the first Resnet layer of the CycleGAN Generators
...
UNET_BATCH_SIZE = 5                                                         # Batch size used during UNet training
...
if not isinstance(USE_GPUS_NO, list) or isinstance(USE_GPUS_NO, tuple):
    USE_GPUS_NO = (USE_GPUS_NO, )
os.environ['CUDA_VISIBLE_DEVICES'] = ','.join((str(i) for i in USE_GPUS_NO))
...
def start_step_2():
    ...
    w_gan.simulate_masks(no_of_images=num_masks,            # No of fake masks to simulate
                         ...
                         max_overlap=MAX_PARTICLE_OVERLAP,  # Maximum overlap of particles in simulated masks (fraction of particle area) - Use a medium to high value for SEM images (e.g., 0.5) and a small value for TEM images (e.g., 0.05)
...
def start_step_3():
    ...
    cycle_gan.gaussian_noise_value = 0.0                    # Set to a small value (e.g., 0.15) to add Gaussian Noise to the discriminator layers (can help against mode collapse and "overtraining" the discriminator)
    ...
    cycle_gan.lambda_identity_a = 0.5                       # CycleGAN Identity Loss for generator a
    cycle_gan.lambda_identity_b = 0.5                       # CycleGAN Identity Loss for generator b
    cycle_gan.filters = CYCLEGAN_FILTERS                    # Number of filters in first layer of the generators
    cycle_gan.use_binary_crossentropy = False               # Use Binary Crossentropy Loss (instead of MAE) for the generator producing fake masks
    cycle_gan.use_resize_convolution = False                # Use Resize followed by regular Convolution instead of "Transpose Convolutions" (can reduce checkerboard artifacts, but usually results in blurring)
...
def start_step_4():
    ...
    cycle_gan = CycleGAN.CycleGAN(root_dir=ROOT_DIR, image_shape=(TILE_SIZE_H, TILE_SIZE_W, 1), allow_memory_growth=ALLOW_MEMORY_GROWTH, use_gpus_no=USE_GPUS_NO)
    cycle_gan.use_skip_connection = CYCLEGAN_USE_SKIPS      # Add a skip connection between the input and output layer in the generator (conceptually similar to identity mappings)
    cycle_gan.filters = CYCLEGAN_FILTERS
    cycle_gan.use_binary_crossentropy = False
    cycle_gan.use_resize_convolution = False
...
def start_step_6b():
    print('Step 6.b: Segment real images with UNet')
    u_net = UNet_Segmentation.UNet(root_dir=ROOT_DIR, image_dir=os.path.join(ROOT_DIR, '2_CycleGAN', 'generate_images', 'A'), mask_dir=os.path.join(ROOT_DIR, '2_CycleGAN', 'generate_images', 'Synthetic_Masks_Filtered'), allow_memory_growth=ALLOW_MEMORY_GROWTH, use_gpus_no=USE_GPUS_NO)
    u_net.use_dataloader = USE_DATALOADER                                    # Whether to use a dataloader
    u_net.filters = UNET_FILTERS                                             # Filters in the first layer of the UNet
