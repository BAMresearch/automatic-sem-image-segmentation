...
class ImageDataset:
    def __init__(self, image_dir, mask_dir, contrast_optimization_range=(0.5, 99.5), use_brightness_and_contrast_augmentation=False):
        self.image_ids = []
        self.image_info = {}
        self.type = ''
        ...
        self.use_brightness_and_contrast_augmentation = use_brightness_and_contrast_augmentation
    ...
    def load_from_file(self, image_ids, is_mask):
        ...
        for image_id in image_ids:
            ...
            if is_mask:
                ...
                if self.type == 'train':
                    if self.use_brightness_and_contrast_augmentation:
                        c_opt = random.random()*2
                        image = HelperFunctions.load_and_preprocess_images(info['image_path'], normalization_range=(0-random.random(), 1+random.random()), contrast_optimization_range=(c_opt, c_opt+98))[0]
                        image -= np.min(image)
                        image /= np.max(image)
                    else:
...
class DataSet(keras.utils.Sequence):
    def __init__(self, x, y, batch_size=1, shuffle=True, **kwargs):
...
class UNet:
    ...
    def step_decay(self, epoch, current_lr, drop=0.5, epochs_drop=10):
    ...
    def run_training(self):
        ...
        # Callbacks
        checkpoint_path = os.path.join(self.model_dir, self.prefix, "Checkpoint_Lowest_Loss.keras")
        ...
        if self.lr_decay == 'STEP_DECAY':
            rate_scheduler = keras.callbacks.LearningRateScheduler(self.step_decay)
        ...
        # Save the model
        print('Saving model to: ' + str(os.path.join(self.model_dir, self.prefix, 'model.keras')))
    ...
    def run_inference(self, files, output_directory, model=None, tile_images=False, threshold=-1, watershed_lines=True, min_distance=9, min_overlap=2, manage_overlap_mode=2, use_gpu=False):
        if self.model is None:
            ...
            unweighted_bce = keras.losses.BinaryCrossentropy(from_logits=False, reduction='none')
            weighting = 1  # Not used for inference, just set to 1

            def weighted_bce(y_true, y_pred):
                weights = (y_true * (weighting - 1)) + 1
                bce = keras.ops.expand_dims(unweighted_bce(y_true, y_pred), -1)
                return keras.ops.mean(bce * weights)

            if model is None:
                # Load the most recent model
                self.model = keras.models.load_model(os.path.join(self.model_dir, os.listdir(self.model_dir)[-1], 'model.keras'), custom_objects={'weighted_bce': weighted_bce})
            elif isinstance(model, str):
                # Load the specified model
                self.model = keras.models.load_model(model, custom_objects={'weighted_bce': weighted_bce})
            else:
                self.model = model
        else:
            self.model = model
        ...
        if not tile_images and input_files[0].shape != self.image_shape:
            input_img = keras.layers.Input(shape=(input_files.shape[1], input_files.shape[2], 1))
            multires_unet = UNet.multi_res_unet(input_img, output_channels=1, conv_filters=self.filters)
            model_new = keras.models.Model(input_img, multires_unet)
        ...
        with keras.device(device):
            for i in tqdm(range(0, input_files.shape[0])):
                ...
                if tile_images:
                    tiles = np.array(HelperFunctions.tile_image(input_file, self.image_shape[0], self.image_shape[1], min_overlap=min_overlap))
                    prediction = np.array([UNet.to_numpy_array(self.model(img.reshape(1, img.shape[0], img.shape[1], img.shape[2]), training=False))[0] for img in tiles])
                    ...
                    prediction = UNet.to_numpy_array(self.model(input_file.reshape(1, input_file.shape[0], input_file.shape[1], input_file.shape[2]), training=False))
    ...
    def create_model(self):
        ...
        input_layer = keras.layers.Input(shape=(self.image_shape[0], self.image_shape[1], 1))
        multires_unet = UNet.multi_res_unet(input_layer, output_channels=self.output_channels, conv_filters=self.filters)
        model = keras.models.Model(input_layer, multires_unet)

        if isinstance(self.lr_decay, float):
            opt = keras.optimizers.Adam(learning_rate=self.learning_rate, weight_decay=self.lr_decay)
        else:
            opt = keras.optimizers.Adam(learning_rate=self.learning_rate, weight_decay=0.0)
    ...
    def multi_res_unet(inputs, output_channels=1, conv_filters=16):
        ...
        Arguments:
            inputs {keras.layers.Input} -- Input Layer
        ...
        up6 = keras.layers.concatenate([keras.layers.Conv2DTranspose(filters*8, (2, 2), strides=(2, 2), padding='same')(mresblock5), mresblock4], axis=3)
        mresblock6 = UNet.multi_res_block(32 * 8, up6)

        up7 = keras.layers.concatenate([keras.layers.Conv2DTranspose(filters*4, (2, 2), strides=(2, 2), padding='same')(mresblock6), mresblock3], axis=3)
        mresblock7 = UNet.multi_res_block(32 * 4, up7)

        up8 = keras.layers.concatenate([keras.layers.Conv2DTranspose(filters*2, (2, 2), strides=(2, 2), padding='same')(mresblock7), mresblock2], axis=3)
        mresblock8 = UNet.multi_res_block(32 * 2, up8)

        up9 = keras.layers.concatenate([keras.layers.Conv2DTranspose(filters, (2, 2), strides=(2, 2), padding='same')(mresblock8), mresblock1], axis=3)
        mresblock9 = UNet.multi_res_block(filters, up9)

        cropped = keras.layers.Cropping2D(cropping=((padding_height // 2, padding_height // 2 + padding_height % 2), (padding_width // 2, padding_width // 2 + padding_width % 2)))(mresblock9)

        if output_channels == 1:
            conv10 = UNet.conv2d_bn(cropped, 1, 1, 1, activation='sigmoid', name='output')
        else:
            conv10 = keras.layers.Conv2D(output_channels, (1, 1), strides=1, activation=None)(cropped)
