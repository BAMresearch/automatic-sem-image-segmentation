...
inputModelPath = '.\\Model.h5'
outputModelPath = inputModelPath.replace('.h5', '.pb')

### Workaround for models created with Tensorflow 2.x:
# 1. Save Model Weights from Tensorflow 2.x after training using model.save_weights
# 2. Build the same Model in Tensorflow 1.x
# 3. Load the weights
# 4. Save as a Tensorflow 1.x Model
# 5. Proceed as usual

inp = Input(shape=(512, 352, 1))
m = MultiResUNet.MultiResUNet.MultiResUnet(inp)
model = Model(inp, m)
model.load_weights(inputModelPath.replace('.h5', '_weights.h5'))
model.save(inputModelPath)
...
flags.DEFINE_string('input_model', inputModelPath, 'Path to the input model.')
...
flags.DEFINE_string('output_model', outputModelPath, 'Path where the converted model will '
                                          'be stored.')
...
def load_model(input_model_path, input_json_path=None, input_yaml_path=None):
    ...
    try:
        model = keras.models.load_model(input_model_path, custom_objects={'weighted_bce': weighted_bce, 'weighted_cce': weighted_cce})
        return model
